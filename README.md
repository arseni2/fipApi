...
## Маршруты на фронтенде

Приложение использует следующие маршруты:

- `/` - Главная страница с защитой для авторизованных пользователей. Здесь пользователь может видеть список своих досок и управлять ими.
- `/dashboard/:id` - Страница детального просмотра конкретной доски с защитой для авторизованных пользователей. Здесь можно редактировать содержимое доски.
- `/board/:publicHash` - Публичная страница доски, доступная по уникальному хэшу без необходимости авторизации. Позволяет просматривать содержимое доски.
- `/signin` - Страница входа в систему, где пользователь может войти с помощью email и пароля.
- `/signup` - Страница регистрации нового пользователя, где можно создать новый аккаунт.
